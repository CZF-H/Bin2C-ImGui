...
#include "jiangzhi_impl_hookapp_android.h"
#include <ctime>
...
int32_t ImGui_ImplAndroid_HandleInputEvent(const AInputEvent* input_event, const ImVec2& screen_scale) {
	...
	switch (event_type) {
		...
		case AINPUT_EVENT_TYPE_MOTION: {
			...
			switch (AMotionEvent_getToolType(input_event, event_pointer_index)) {
				case AMOTION_EVENT_TOOL_TYPE_MOUSE: io.AddMouseSourceEvent(ImGuiMouseSource_Mouse);
					break;
				case AMOTION_EVENT_TOOL_TYPE_STYLUS:
				case AMOTION_EVENT_TOOL_TYPE_ERASER: io.AddMouseSourceEvent(ImGuiMouseSource_Pen);
					break;
				case AMOTION_EVENT_TOOL_TYPE_FINGER:
				default: io.AddMouseSourceEvent(ImGuiMouseSource_TouchScreen);
					break;
			}
			...
			switch (event_action) {
				...
				case AMOTION_EVENT_ACTION_UP: {
					...
					if (tool_type == AMOTION_EVENT_TOOL_TYPE_FINGER ||
						...
						io.AddMousePosEvent(
							screen_scale.x > 0 ? raw_pos.x / screen_scale.x : raw_pos.x,
							screen_scale.y > 0 ? raw_pos.y / screen_scale.y : raw_pos.y
							); // 传入缩放后
...
bool ImGui_ImplAndroid_Init(ANativeWindow* window) {
	...
	ImGuiIO& io = ImGui::GetIO();
	io.BackendPlatformName = "jiangzhi_impl_hookapp_android";
...
void ImGui_ImplAndroid_NewFrame(int screen_width, int screen_height) {
	...
	int32_t window_width = screen_width <= 0 ? ANativeWindow_getWidth(g_Window) : screen_width;
	int32_t window_height = screen_height <= 0 ? ANativeWindow_getHeight(g_Window) : screen_height;
	...
	if (window_width > 0 && window_height > 0)
		io.DisplayFramebufferScale = ImVec2(
			(float) display_width / static_cast<float>(window_width),
			(float) display_height / static_cast<float>(window_height));

	// Setup time step
	struct timespec current_timespec{ };
	clock_gettime(CLOCK_MONOTONIC, &current_timespec);
	double current_time = (double) (current_timespec.tv_sec) +
	                      (static_cast<double>(current_timespec.tv_nsec) / 1000000000.0);
